...
extension Array where Element == Limb {
    ...
    static func divideBy3(_ d: BInt) -> BInt {
        var quotient = Limbs(repeating: 0, count: d._magnitude.count)
        var remainder = Limb(0)
        for i in 0 ..< quotient.count {
            let w = remainder > d._magnitude[i] ? remainder - d._magnitude[i] : d._magnitude[i] - remainder
    ...
    func toomCookTimes(_ x: Limbs) -> Limbs {
        ...
        var offset = k << 2
        result.add(rinf._magnitude, offset)
        offset -= k
        result.add(rr3._magnitude, offset)
        offset -= k
        result.add(rr2._magnitude, offset)
        result.add(rr1._magnitude, k)
        result.add(r0._magnitude, 0)
        return result
    }
    ...
    func toomCookSquare() -> Limbs {
        ...
        var result = Limbs(repeating: 0, count: 6 * k)
        var offset = k << 2
        result.add(vinf._magnitude, offset)
        offset -= k
        result.add(t2._magnitude, offset)
        offset -= k
        result.add(t1._magnitude, offset)
        result.add(tm1._magnitude, k)
        result.add(v0._magnitude, 0)
        return result
    }
